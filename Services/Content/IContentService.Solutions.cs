...
public partial interface IContentService
{
    ...
    Task<IReadOnlyList<IContentItem<Solution>>> GetSolutionsAsync(CancellationToken ct = default);
    ...
    /// <exception cref="ContentDeliveryException">Thrown when the Delivery API returns a server error (5xx).</exception>
    Task<IContentItem<Solution>?> GetSolutionBySlugAsync(string slug, CancellationToken ct = default);
}
